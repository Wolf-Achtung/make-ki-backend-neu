...
# -*- coding: utf-8 -*-
"""
KI-Status-Report Analyzer – Gold-Standard+ (branchen-booster & baselines)
- Branchen-Booster (DE/EN) aus data/branch_boosters.*.json in Quick Wins, Recommendations, Praxisbeispiel
- Baseline-Daten für Förderungen/Tools (CSV) werden *immer* ergänzt (vor Live-Layer)
- Content-Einbindung (Guides & Checklisten) via {{CONTENT_*}} Platzhalter (DE: DOCX > HTML, EN: *.en.html)
- Hybrid Live-Layer (Perplexity Search + Tavily) wie gehabt; Guards in websearch_utils/perplexity_client
"""
...
def _read_text(path: Path) -> str:
    try:
        ...
        log.warning("read failed %s: %s", path, exc); return ""
...
def _load_prompt(lang: str, name: str) -> str:
    ...
    for p in cand:
        if p.exists(): log.info("Loaded prompt: %s", p.relative_to(PROMPTS_DIR)); return _read_text(p)
    log.info("Prompt missing for '%s' (%s) – skipping section", name, lang); return ""

def _load_branch_booster_text(lang: str, branche_code: str) -> str:
    code = (branche_code or "default").strip().lower()
    fname = "branch_boosters.de.json" if str(lang).lower().startswith("de") else "branch_boosters.en.json"
    p = DATA_DIR / fname
    try:
        if p.exists():
            obj = json.loads(_read_text(p) or "{}")
            # try both exact code and common English variants
            for key in (code, {"beratung":"consulting","it":"it","marketing":"marketing","bau":"construction","industrie":"industry",
                               "handel":"trade","finanzen":"finance","gesundheit":"healthcare","medien":"media","logistik":"logistics",
                               "verwaltung":"public administration","bildung":"education"}.get(code, code), "default"):
                if key in obj and obj[key]:
                    return str(obj[key])
    except Exception as exc:
        log.warning("booster load failed: %s", exc)
    return ""
...
def render_overlay(name: str, lang: str, ctx: Dict[str, Any]) -> str:
    ...
    # Branch booster
    bcode = (ctx.get("briefing") or {}).get("branche") or "default"
    booster = _load_branch_booster_text(lang, bcode)
    ...
    user = (prompt
        ...
        .replace("{{BRANCH_SNIPPET}}", booster)
    )
    if name in {"praxisbeispiel","quick_wins","recommendations","roadmap"} and booster:
        user = user + "\n\n[Branchen-Booster]\n" + booster
...
# ---------------- Content loader (Guides & Checklisten) ----------------

def _docx_to_html(path: Path) -> str:
    """Very light-weight DOCX->HTML (paragraph text only, headings as <h4>)."""
    try:
        ...
        for p in root.findall(".//w:p", ns):
            ...
            if style is not None and style.attrib.get(f"{{{ns['w']}}}val","").lower().startswith("heading"):
                parts.append(f"<h4>{para}</h4>")
            else:
                parts.append(f"<p>{para}</p>")
        return "\n".join(parts)
    except Exception as exc:
        log.warning("docx parse failed for %s: %s", path, exc); return ""

def load_guides_content(lang: str) -> Dict[str, str]:
    out = {"pillars": "", "legal": "", "formula": ""}
    if str(lang).lower().startswith("de"):
        # prefer DE docx files; fall back to EN html
        de_map = {
        ...
        for key, files in de_map.items():
            for fn in files:
                p = CONTENT_DIR / fn
                if p.exists():
                    out[key] = _docx_to_html(p)
                    break
    else:
        en_map = {
            "pillars": ["4-pillars-ai-readiness.en.html"],
            "legal": ["legal-pitfalls-ai.en.html"],
            "formula": ["transformation-formula-10-20-70.en.html"],
        }
        for key, files in en_map.items():
            for fn in files:
                p = CONTENT_DIR / fn
                if p.exists():
                    out[key] = _as_fragment(_read_text(p))
                    break
    return out
...
def _read_csv(path: Path) -> List[Dict[str, str]]:
    try:
        ...
        log.warning("csv read failed %s: %s", path, exc); return []

def _domain_from_url(url: str) -> str:
    if not url or "://" not in url: return ""
    return url.split("/")[2]

def load_baseline_funding(n: Normalized, max_items: int = 8) -> List[Dict[str, Any]]:
    p = DATA_DIR / "foerdermittel.csv"
    rows = _read_csv(p) if p.exists() else []
    out: List[Dict[str, Any]] = []
    reg = (n.bundesland_code or "").lower()
    for r in rows:
        region = (r.get("region") or "").lower()
        # take national/EU entries plus region match (loose filter)
        if region not in {"bund", "de", "deutschland", "eu"} and region != reg:
            # allow anyway if small baseline; do not overfilter
            pass
        url = r.get("link") or ""
        title = r.get("name") or "Förderprogramm"
        out.append({
            "title": title, "url": url, "domain": _domain_from_url(url),
            "date": ""  # baseline has no recency
        })
    return out[:max_items]

def load_baseline_tools(n: Normalized, max_items: int = 8) -> List[Dict[str, Any]]:
    ...
    for r in rows:
        slugs = (r.get("Branche-Slugs") or "").lower()
        sizes = (r.get("Unternehmensgröße") or r.get("Unternehmensgroesse") or "").lower() or "alle"
        if slugs and code not in slugs and "default" not in slugs and "alle" not in slugs:
            continue
        if sizes not in {"alle", size} and not (sizes == "kmu" and size in {"team","kmu"}):
            continue
        url = r.get("link") or ""
        title = r.get("name") or "Tool"
        out.append({
            "title": title, "url": url, "domain": _domain_from_url(url),
            "date": "", "category": r.get("kategorie") or "", "price": r.get("preis") or ""
        })
    return out[:max_items]
...
def _profile_html(n: Normalized) -> str:
    ...
    return ("<p><span class='hl'>Hauptleistung:</span> {hl} "
            "<span class='muted'>&middot; Branche:</span> {br} "
            "<span class='muted'>&middot; Größe:</span> {sz}</p><p>{pills}</p>").format(
                hl=n.hauptleistung, br=n.branche_label, sz=n.unternehmensgroesse_label, pills=pills_html)
...
def build_html_report(raw: Dict[str, Any], lang: str = "de") -> Dict[str, Any]:
    # Boot snapshot – helps diagnose env/model in logs
    _emit("analyzer", None, "boot", 0, 0, extra={
        "hybrid_live": os.getenv("HYBRID_LIVE", "1"),
        "pplx_model_effective": (os.getenv("PPLX_MODEL") or "").strip() or "none",
    ...
    # Baseline funding/tools from CSV
    baseline_funding = load_baseline_funding(n, max_items=max(LIVE_MAX_ITEMS, 10))
    baseline_tools = load_baseline_tools(n, max_items=LIVE_MAX_ITEMS)

    # Live (hybrid) – Perplexity Search + Tavily
    ...
    news = []; tools = list(baseline_tools); funding = list(baseline_funding)
    if websearch_utils:
        try:
            news_live = websearch_utils.perplexity_search(q_news, max_results=LIVE_MAX_ITEMS) + \
                        websearch_utils.tavily_search(q_news, max_results=LIVE_MAX_ITEMS, days=SEARCH_DAYS_NEWS)
            tools_live = websearch_utils.perplexity_search(q_tools, max_results=LIVE_MAX_ITEMS) + \
                         websearch_utils.tavily_search(q_tools, max_results=LIVE_MAX_ITEMS, days=SEARCH_DAYS_TOOLS)
            fund_live = websearch_utils.perplexity_search(q_fund, max_results=max(LIVE_MAX_ITEMS, 10)) + \
                        websearch_utils.tavily_search(q_fund, max_results=max(LIVE_MAX_ITEMS, 10), days=SEARCH_DAYS_FUNDING)
            news = news_live
            tools.extend(tools_live)
            funding.extend(fund_live)
        except Exception as exc:  # pragma: no cover
    ...
    # Guides & Checklists
    guides = load_guides_content(lang)
    ...
    html = (tpl.replace("{{LANG}}", "de" if lang.startswith("de") else "en")
           ...
           .replace("{{CONTENT_PILLARS_HTML}}", guides.get("pillars",""))
           .replace("{{CONTENT_LEGAL_HTML}}", guides.get("legal",""))
           .replace("{{CONTENT_FORMULA_HTML}}", guides.get("formula",""))
