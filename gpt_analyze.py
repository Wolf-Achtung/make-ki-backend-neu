...
def generate_full_report(data: dict, lang: str = "de") -> dict:
    ...
    # --- Gold-Standard: Vision normalisieren + narrative Felder setzen ---
    # 1) Vision: altes Label "Kühne Idee"/"Bold idea" -> "Vision"
    try:
        if isinstance(out.get("vision"), str):
            vtxt = out["vision"]
            for legacy in ("Kühne Idee:", "Kühne Idee", "Bold idea:", "Bold idea"):
    ...
    # 2) Narrative statt Tabellen bevorzugen (falls LLM-Text geliefert hat)
    try:
    ...
    # 3) Compliance: eigener narrativer Block sicherstellen
    def _fallback_compliance_narr(_lang: str) -> str:
        if (_lang or "de").startswith("de"):
            return (
                "<p>Für den rechtssicheren KI-Einsatz sind DSGVO, ePrivacy, Digital Services Act "
                "und der EU AI Act zentral. Empfohlen sind Privacy-by-Design, Datenminimierung, "
                "Rollen-/Rechtekonzept, eine kurze DSFA bei erhöhtem Risiko sowie nachvollziehbare "
                "Freigaben (Human-in-the-Loop) und Audit-Trails.</p>"
                "<p>Praktisch heißt das: Datenflüsse dokumentieren, Auftragsverarbeiter prüfen, "
                "Modelle/Prompts versionieren, Logging & Monitoring etablieren und Betroffenenrechte "
                "effizient bedienen – für eine belastbare, auditierbare Basis.</p>"
            )
        else:
            return (
                "<p>For compliant AI, focus on GDPR, ePrivacy, the Digital Services Act and the EU AI Act. "
                "Apply privacy-by-design, data minimisation, role-based access, light DPIA for higher risks, "
                "human-in-the-loop approvals and audit trails.</p>"
                "<p>In practice: document data flows, verify processors, version models and prompts, enable "
                "logging & monitoring and handle data-subject rights efficiently to stay auditable.</p>"
            )
