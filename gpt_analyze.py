# gpt_analyze.py — Analyzer (Gold-Standard+ with content injection & badges)
...
try:
    from live_logger import log_event as _emit
except Exception:
...
def _openai_chat(messages: List[Dict[str,str]], model: Optional[str] = None, max_tokens: Optional[int] = None) -> str:
    ...
    payload = {"model": model or OPENAI_MODEL, "messages": messages, "max_tokens": int(max_tokens or OPENAI_MAX_TOKENS), "temperature": GPT_TEMPERATURE, "top_p": 0.95}
    try:
...
def _load_snippet(lang: str, branche: str) -> str:
    # Optional Branchen-Booster
    p = PROMPTS_DIR/"_snippets"/f"{branche}.{ 'de' if lang.startswith('de') else 'en'}.md"
    return _read_text(p) if p.exists() else ""

def render_overlay(name: str, lang: str, ctx: Dict[str,Any], branche: str) -> str:
    prompt = _load_prompt(lang, name)
    if not prompt: return ""
    snippet = _load_snippet(lang, branche)
    if snippet:
        prompt = prompt + "\n\n<!-- Branchen-Booster -->\n" + snippet
    ...
    out = _openai_chat([{ "role": "system", "content": system}, {"role":"user","content": user}], model=(EXEC_SUMMARY_MODEL if name=="executive_summary" else OPENAI_MODEL))
    return _as_fragment(out)

# ---- Schema/Normalization/Scoring/ROI (unchanged essentials) ----
...
def normalize_briefing(raw: Dict[str,Any], lang: str = "de") -> Normalized:
    ...
    def _derive_kpis(bb: Dict[str,Any]) -> Dict[str,int]:
        ...
        if isinstance(bb.get("ki_einsatz"), list) and bb["ki_einsatz"]: auto = min(100, auto+5)
        comp = 40
...
# ---- Content injection (HTML files) ----
def _load_content_blocks(lang: str) -> str:
    blocks: List[str] = []
    cand = [
        ("4-pillars-ai-readiness.en.html", "4 Säulen Ihrer KI‑Readiness" if lang.startswith("de") else "4 Pillars of AI Readiness"),
        ("legal-pitfalls-ai.en.html", "Rechtliche Stolpersteine" if lang.startswith("de") else "Legal Pitfalls When Using AI"),
        ("transformation-formula-10-20-70.en.html", "Transformationsformel 10‑20‑70" if lang.startswith("de") else "Transformation Formula 10‑20‑70"),
    ]
    for fname, title in cand:
        p = CONTENT_DIR / fname
        if not p.exists(): 
            continue
        html = _as_fragment(_read_text(p))
        if lang.startswith("de") and fname.endswith(".en.html"):
            # simple heading localisation
            html = (html.replace("<h2>Legal Pitfalls When Using AI in Companies</h2>", "<h2>Rechtliche Stolpersteine bei KI im Unternehmen</h2>")
                        .replace("<h2>4 Pillars of AI Readiness</h2>", "<h2>Die 4 Säulen Ihrer KI‑Readiness</h2>")
                        .replace("Transformation Formula:", "Transformationsformel:"))
        blocks.append(f"<section class='guide'>{html}</section>")
    return "\n".join(blocks)
...
def build_html_report(raw: Dict[str,Any], lang: str = "de") -> Dict[str,Any]:
    ...
    if websearch_utils and HYBRID_LIVE:
        try:
            news = websearch_utils.perplexity_search(q_news, max_results=LIVE_MAX_ITEMS, days=SEARCH_DAYS_NEWS) + websearch_utils.tavily_search(q_news, max_results=LIVE_MAX_ITEMS, days=SEARCH_DAYS_NEWS)
            tools = websearch_utils.perplexity_search(q_tools, max_results=LIVE_MAX_ITEMS, days=SEARCH_DAYS_TOOLS) + websearch_utils.tavily_search(q_tools, max_results=LIVE_MAX_ITEMS, days=SEARCH_DAYS_TOOLS)
            funding = websearch_utils.perplexity_search(q_fund, max_results=max(LIVE_MAX_ITEMS,10), days=SEARCH_DAYS_FUNDING) + websearch_utils.tavily_search(q_fund, max_results=max(LIVE_MAX_ITEMS,10), days=SEARCH_DAYS_FUNDING)
        except Exception as exc:
    ...
    sec = lambda name: render_overlay(name, lang, ctx, n.branche) or ""
    ...
    guides = _load_content_blocks(lang)
    ...
    html = (tpl.replace("{{LANG}}", "de" if lang.startswith("de") else "en")
        ...
        .replace("{{CONTENT_BLOCKS_HTML}}", guides)
    )
