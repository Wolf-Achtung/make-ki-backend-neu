...
def fallback_vision(data: dict, lang: str = "de") -> str:
    ...
    # update the wording to suit your industry context.
    if lang == "de":
        paragraphs = [
            ("<p><b>Vision:</b> KI‑Serviceportal für KMU – ein digitales Ökosystem, das "
        ...
        paragraphs = [
            ("<p><b>Vision:</b> AI service portal for SMEs – a digital ecosystem that opens up "
...
def _toc_from_report(report: Dict[str, Any], lang: str) -> str:
    ...
    if lang == "de":
        ...
        # Include funding programmes and tools if either a narrative or table is present
        if report.get("foerderprogramme_html") and str(report.get("foerderprogramme_html")).strip():
            toc_items.append("<li>Förderprogramme</li>")
        elif report.get("foerderprogramme_table"):
            toc_items.append("<li>Förderprogramme</li>")
        if report.get("tools_html") and str(report.get("tools_html")).strip():
            toc_items.append("<li>KI-Tools & Software</li>")
        elif report.get("tools_table"):
            toc_items.append("<li>KI-Tools & Software</li>")
...
def generate_full_report(data: dict, lang: str = "de") -> dict:
    ...
    #
    # The PDF template prefers narrative paragraphs for these sections.  The
    # original LLM-generated HTML is stored under the chapter keys
    # "foerderprogramme", "tools" and "compliance".  To honour the new
    # narrative design we forward these strings into dedicated *_html fields.  If
    # a section is empty or missing the HTML value will be an empty string,
    # prompting the template to fall back to the tabular display.
    try:
        # Förderprogramme narrative
        fp_raw = out.get("foerderprogramme") or ""
        fp_html = ensure_html(strip_code_fences(fix_encoding(fp_raw)), lang) if fp_raw else ""
        out["foerderprogramme_html"] = fp_html
    except Exception:
        out["foerderprogramme_html"] = ""
    try:
        # Tools narrative
        tl_raw = out.get("tools") or ""
        tl_html = ensure_html(strip_code_fences(fix_encoding(tl_raw)), lang) if tl_raw else ""
        out["tools_html"] = tl_html
    except Exception:
        out["tools_html"] = ""
    try:
        # Compliance narrative
        comp_raw = out.get("compliance") or ""
        comp_html = ensure_html(strip_code_fences(fix_encoding(comp_raw)), lang) if comp_raw else ""
        out["compliance_html"] = comp_html
    except Exception:
        out["compliance_html"] = ""
