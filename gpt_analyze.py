...
import glob, io, os, re, json, logging
...
DEFAULT_BENCHMARK_FILE = os.path.join(BENCHMARKS_DIR, "default.json")
...
# ----------------------------- Benchmarks -----------------------------------
def load_default_benchmarks() -> Dict[str, Any]:
    """
    Lädt Benchmarks aus data/benchmarks/default.json. Fällt andernfalls auf neutrale Defaults zurück.
    """
    try:
        with io.open(DEFAULT_BENCHMARK_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            if isinstance(data, dict) and data:
                return data
    except Exception as e:
        log.info("Benchmarks: fallback to internal defaults (%s)", e)
    # neutrale, DE/EN-konforme Startwerte
    return {
        "neutral": {
            "digitalization": 60,
            "automation": 55,
...
def build_readiness_benchmark(answers: Dict[str, Any], kpis: Dict[str, Any], lang: str) -> Dict[str, Any]:
    """
    Baut eine kompakte Vergleichstabelle (unsere Werte vs. Benchmark) für das Readiness‑Kapitel.
    """
    bm_all = load_default_benchmarks()
    branch = str(answers.get("branche", "neutral")).lower()
    bm = bm_all.get(branch) or bm_all.get("neutral", {})

    def bval(key: str, default: int) -> int:
        return _safe_int(bm.get(key, default), default)

    # Unsere Werte als Prozent
    digital_pct = min(100, max(0, _safe_int(answers.get("digitalisierungsgrad", 6), 6) * 10))
    auto_pct = _safe_int(kpis.get("automatisierungsgrad", 50), 50)
    paper_map = {"0-20": 20, "21-50": 40, "51-80": 65, "81-100": 90}
    paper_pct = paper_map.get(str(answers.get("prozesse_papierlos", "51-80")).replace("_", "-"), 65)
    kw_map = {"anfaenger": 25, "anfänger": 25, "grundkenntnisse": 45, "fortgeschritten": 70, "experte": 90}
    kw_pct = kw_map.get(str(answers.get("ki_knowhow", "grundkenntnisse")).lower(), 45)
    eff_pct = _safe_int(kpis.get("kpi_efficiency", 40), 40)
    comp_pct = _safe_int(kpis.get("kpi_compliance", 70), 70)
    inno_pct = _safe_int(kpis.get("kpi_innovation", 65), 65)

    def row(label_de: str, label_en: str, our: int, bench: int):
        delta = int(our - bench)
        if lang.startswith("de"):
            interp = "Ausbauen" if delta >= 0 else "Aufholen"
            kpi = label_de
        else:
            interp = "Expand" if delta >= 0 else "Catch up"
            kpi = label_en
        return {
            "kpi": kpi,
            "our": f"{our}%",
            "benchmark": f"{bench}%",
            "delta": f"{'+' if delta >= 0 else ''}{delta} pp",
            "interpretation": interp,
        }

    rows = [
        row("Digitalisierung", "Digitalization", digital_pct, bval("digitalization", 60)),
        row("Automatisierung", "Automation", auto_pct, bval("automation", 55)),
        row("Papierlos", "Paperless", paper_pct, bval("paperless", 60)),
        row("KI‑Know-how", "AI know-how", kw_pct, bval("ai_knowhow", 55)),
        row("Effizienzpotenzial", "Efficiency", eff_pct, bval("efficiency", 50)),
        row("Compliance", "Compliance", comp_pct, bval("compliance", 70)),
        row("Innovation", "Innovation", inno_pct, bval("innovation", 60)),
    ]

    note = ("Vergleichsbasis: neutrale Benchmarks (Startpaket). "
            "Eigene Branchenwerte können über data/benchmarks ergänzt werden."
            if lang.startswith("de")
            else "Baseline: neutral starter benchmarks. Add sector files under data/benchmarks.")

    return {"benchmark_table": rows, "benchmark_note": note}
...
def get_template_variables(form_data: Dict[str, Any], lang: str='de') -> Dict[str, Any]:
    ...
    bm = build_readiness_benchmark(form_data, k, lang)
    ...
    vars = {
        ...
        # Benchmarks (neu)
        "benchmark_table": bm["benchmark_table"],
        "benchmark_note": bm["benchmark_note"],

        # Admin‑Payload (Rohdaten)
        "admin_raw_answers_json": json.dumps(form_data, ensure_ascii=False, indent=2)[:50000],
        "admin_kpis_json": json.dumps(k, ensure_ascii=False, indent=2),
