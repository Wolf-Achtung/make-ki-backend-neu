...
def build_context(data: dict, branche: str, lang: str = "de") -> dict:
    ...
    # ------------------------------------------------------------------
    # Berechne vier Kern-Kennzahlen (Digitalisierungsgrad, Automatisierungsgrad,
    # papierlose Prozesse und KI-Know-how) aus den rohen Fragebogendaten.
    # Diese Werte werden als Prozentzahlen gespeichert, damit sie in
    # Vorlagen (z. B. im Prompt-Prefix oder der Executive Summary) angezeigt
    # werden können.  Die Berechnung orientiert sich an den gleichen
    # Mapping-Tabellen wie in der Reportausgabe, ist jedoch leichtgewichtig
    # implementiert, um vom Kontext unabhängig zu sein.
    try:
        # Hilfsfunktion, die versucht, einen numerischen Wert in einen
        # Prozentbereich zu konvertieren.  Wenn der Wert <= 1 ist,
        # interpretieren wir ihn als 0–1-Skala und skalieren auf 0–100.
        # Wenn der Wert <= 10 und eine Ganzzahl (Schieberegler), wird auf
        # 0–100 skaliert.  Andernfalls wird der Wert als Prozent verwendet.
        def _norm_to_percent(val):
            try:
                s = str(val).strip().replace(",", ".")
                num = float(s)
            except Exception:
                return None
            # Werte zwischen 0 und 1 → Prozentskala 0–100
            if num <= 1.0:
                return int(round(num * 100.0))
            # Werte zwischen 1 und 10 (z. B. Slider) → 0–100
            if num <= 10 and abs(num - int(num)) < 1e-6:
                return int(round(num * 10.0))
            # Ansonsten direkt verwenden, aber kappen auf 0–100
            return int(max(0, min(100, round(num))))

        # Mapping-Tabellen für verbale Angaben
        auto_mapping = {
            "gar nicht": 0,
            "nicht": 0,
            "eher niedrig": 25,
        ...
        def _map_scale_text(value, mapping):
            if not value:
                return None
            ...
            if key in mapping:
                return mapping[key]
            # fuzzy: falls ein Mapping-Schlüssel Teil des Antworttexts ist
            for k in sorted(mapping.keys(), key=len, reverse=True):
                if k and k in key:
                    return mapping[k]
            return None

        # Werte aus dem Fragebogen lesen
        raw_digi = data.get("digitalisierungsgrad") or data.get("digitalisierungsgrad (%)") or data.get("digitalisierungs_score")
        raw_auto = data.get("automatisierungsgrad") or data.get("automatisierungsgrad (%)") or data.get("automatisierungs_score")
        raw_paper = data.get("prozesse_papierlos") or data.get("papierlos") or data.get("paperless")
        raw_know = data.get("ki_knowhow") or data.get("knowhow") or data.get("ai_knowhow")

        # Berechnete Werte (falls numerisch)
        own_digi = _norm_to_percent(raw_digi) if raw_digi is not None else None
        own_auto = _norm_to_percent(raw_auto) if raw_auto is not None else None
        own_paper = _norm_to_percent(raw_paper) if raw_paper is not None else None
        own_know = _norm_to_percent(raw_know) if raw_know is not None else None

        # Falls numerische Umrechnung fehlschlägt, versuche textliche Mappings
        if own_auto is None:
            own_auto = _map_scale_text(raw_auto, auto_mapping)
        if own_know is None:
            own_know = _map_scale_text(raw_know, know_mapping)
        # Sicherstellen, dass None-Werte durch 0 ersetzt werden
        own_digi = own_digi if own_digi is not None else 0
        own_auto = own_auto if own_auto is not None else 0
        own_paper = own_paper if own_paper is not None else 0
        own_know = own_know if own_know is not None else 0

        # Speichere die Werte als Prozentstrings im Kontext
        context.setdefault("digitalisierungsgrad", f"{own_digi}%")
        context.setdefault("automatisierungsgrad", f"{own_auto}%")
        context.setdefault("prozesse_papierlos", f"{own_paper}%")
        context.setdefault("ki_knowhow", f"{own_know}%")
    except Exception:
        # Fehler ignorieren: Wenn etwas schiefgeht, bleiben die Werte ungesetzt
        pass
    return context
