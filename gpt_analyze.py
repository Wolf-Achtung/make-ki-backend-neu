...
def _jinja_env():
    """
    Construct a Jinja2 environment that loads templates from multiple search paths
    to tolerate different project layouts (templates/, module dir, CWD).
    """
    search_paths = [str(TEMPLATES_DIR), str(BASE_DIR), str(Path.cwd())]
    return Environment(
...
def _pick_template(lang: str) -> Optional[str]:
    """
    Prefer language-specific template names. With a multi-path loader we can return
    just the filename and let Jinja resolve it in templates/, module dir or CWD.
    """
    name = "pdf_template.html" if lang == "de" else "pdf_template_en.html"
    for p in [TEMPLATES_DIR / name, BASE_DIR / name, Path(name)]:
        if p.exists():
            return name
    return None
...
def generate_full_report(data: dict, lang: str = "de") -> dict:
    ...
    # Mirror narrative fields for template compatibility
    out["compliance_html"] = ensure_html(strip_code_fences(fix_encoding(out.get("compliance", ""))), lang)
    out["tools_html"] = ensure_html(strip_code_fences(fix_encoding(out.get("tools", ""))), lang)
    out["foerderprogramme_html"] = ensure_html(strip_code_fences(fix_encoding(out.get("foerderprogramme", ""))), lang)
...
def analyze_briefing(payload: Dict[str, Any], lang: Optional[str] = None) -> Dict[str, Any]:
    ...
    if template_name:
        ...
        ctx = {
            ...
            "compliance_html": report.get("compliance_html","") or report.get("compliance",""),
            "foerderprogramme_html": report.get("foerderprogramme_html","") or report.get("foerderprogramme",""),
            "tools_html": report.get("tools_html","") or report.get("tools",""),
