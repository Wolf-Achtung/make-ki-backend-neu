...
def _resolve_model(wanted: Optional[str]) -> str:
    ...
    return w if w in fallbacks or w in {"gpt-4.1","gpt-4.1-mini"} else fallbacks[0]
...
# **nur** Prozent/Währung entfernen (keine neutralen Zahlen/Zeitangaben!)
...
def _strip_lists_and_numbers(raw: str) -> str:
    """Für narrative Abschnitte. Entfernt Listenmarker & %/Währungszahlen, NICHT generelle Zahlen."""
...
def _tools_fallback(branche: str, size: str, lang: str="de") -> List[Dict[str,str]]:
    # Minimal kuratierte EU-taugliche Beispiele (kannst Du später erweitern)
    lib = [
...
def build_tools_html(data: Dict[str,Any], branche: str, lang: str="de", max_items: int=8) -> str:
    ...
    if lang.startswith("de"):
        ps.append("<p>Auswahl bewährter, EU‑tauglicher Werkzeuge für einen pragmatischen Start:</p>")
        for r in rows:
        ...
        ps.append("<p>A curated selection of EU‑friendly tools to get started quickly:</p>")
        for r in rows:
            p  = f"<p><b>{r['name']}</b> ({r.get('category','')}) – suitable for {r.get('suitable_for','daily work')}. "
            p += f"Data/hosting: {r.get('hosting','n/a')}; price: {r.get('price','n/a')}. "
            if r.get('link'): p += f'<a href="{r["link"]}">Website</a>'
            p += "</p>"; ps.append(p)
    return "\n".join(ps)

# ---------------- Tavily/SerpAPI + Sanitizer ----------------
...
def build_live_updates_html(data: Dict[str, Any], lang: str = "de", max_results: int = 5) -> Tuple[str, str]:
    ...
    queries = [(q_reg, days_default), (q_funding, days_funding), (q_tools, days_tools)]
    ...
    items = _sanitize_live_items(raw, max_items, maxlen, min_score, includes, excludes) if do_sanit else raw[:max_items]
    if not items: return ("","")

    lab = "Neu & relevant" if lang.startswith("de") else "New & relevant"
...
def build_context(data: Dict[str, Any], branche: str, lang: str = "de") -> Dict[str, Any]:
    ...
    bl = str(ctx.get("bundesland") or ctx.get("state") or "").lower()
    ctx["bundesland"] = bl

    # Fallbacks für Coaching/Prompts (niemals Platzhalter!)
...
def _read_prompt(chapter: str, lang: str) -> str:
    ...
    if chapter == "coaching":
        ...
        # Fallback auf Deine gelieferten Dateinamen
...
def build_masterprompt(chapter: str, context: Dict[str,Any], lang: str) -> str:
    ...
    # Sicherheitsregel: niemals Platzhalter, wenn Felder fehlen
    ...
    # Stilregeln (narrativ, kein Listen-Spam)
    rules = ("Gib die Antwort als gültiges HTML (ohne <html>-Wrapper). Nutze <h3> und <p>. "
             ...
             "Keine Prozent- oder Geldzahlen." if lang=="de" else
...
# ---------------- Appendix (nach „bitte-einbauen-am-Ende.pdf“) --------------
...
<section>
  ...
  <h3>Glossar</h3>
  <p><b>KI</b>: Technologien, die aus Daten lernen. <b>DSGVO</b>: EU‑Datenschutzrecht. <b>DSFA</b>: Datenschutz‑Folgenabschätzung. <b>EU AI Act</b>: EU‑Verordnung zu Risikoklassen und Pflichten. <b>Quick Win</b>: schneller Nutzen bei geringem Aufwand. <b>MVP</b>: minimal funktionsfähige erste Version.</p>
...
def generate_full_report(data: Dict[str,Any], lang: str="de") -> Dict[str,Any]:
    ...
    # Executive Summary — sicherstellen, dass etwas da ist
    if not out.get("executive_summary_html"):
        if lang=="de":
            out["executive_summary_html"] = _ensure_html("Kurzüberblick: Status, 3 schnelle Hebel, größte Risiken, nächste Schritte (30–60 Tage).", "de")
        else:
            out["executive_summary_html"] = _ensure_html("Quick brief: current status, 3 quick wins, top risks, next 30–60‑day steps.", "en")
