...
def analyze_briefing(payload: Dict[str, Any], lang: Optional[str] = None) -> Dict[str, Any]:
    ...
    # ---------------------------------------------------------------------
    # Sanitize all HTML fields in the report before rendering.  Remove
    # internal guideline sections ("Zusätzliche Anweisungen" or
    # "Additional Instructions") and drop any second KPI overview that may
    # have been generated by the LLM.  This ensures that the final PDF
    # contains only the intended report content and not any prompt
    # instructions or duplicate summaries.  The sanitisation functions are
    # defined inline to avoid polluting the top level namespace.
    def _strip_internal_guidelines(html: str) -> str:
        """
        Remove internal guideline sections from a rendered HTML fragment.
        Any <h2> or <h3> heading starting with "Zusätzliche Anweisungen"
        (DE) or "Additional Instructions" (EN) and its following
        content up to the next heading is stripped.  This prevents
        prompt guidelines from leaking into the final report.

        Parameters
        ----------
        html : str
            A snippet of HTML content.

        Returns
        -------
        str
            Sanitised HTML with internal guideline sections removed.
        """
        if not isinstance(html, str) or not html:
            return html
        # Pattern to match a guideline section: heading and any following
        # content until the next h2/h3 or end of string.  Use a
        # case-insensitive match and dot-all to capture newlines.
        patterns = [
            r"(?is)<h[23][^>]*>\s*Zusätzliche\s+Anweisungen.*?(?=<h[23]|$)",
            r"(?is)<h[23][^>]*>\s*Additional\s+Instructions.*?(?=<h[23]|$)",
        ]
        sanitized = html
        for pat in patterns:
    ...
    def _strip_kpi_overview(html: str) -> str:
        """
        Remove any secondary KPI overview generated by the LLM.  The first
        KPI summary is generated programmatically; subsequent sections
        headed by <h3>KPI-Überblick</h3> (DE) or <h3>KPI overview</h3> (EN)
        are removed along with the following content until the next
        heading.  This prevents conflicting narratives from appearing in
        the Executive Summary.

        Parameters
        ----------
        html : str
            HTML content that may include unwanted KPI overview sections.

        Returns
        -------
        str
            HTML with secondary KPI overview sections removed.
        """
        if not isinstance(html, str) or not html:
            return html
        sanitized = html
        # German KPI-Überblick removal
        sanitized = re.sub(r"(?is)<h3[^>]*>\s*KPI[\s\-]*Überblick\s*</h3>.*?(?=<h[23]|$)", "", sanitized)
        # English KPI overview removal
        sanitized = re.sub(r"(?is)<h3[^>]*>\s*KPI\s*overview\s*</h3>.*?(?=<h[23]|$)", "", sanitized)
        return sanitized

    # Apply sanitisation to all HTML fields in the report dictionary.
    if isinstance(report, dict):
        html_keys = [
            "exec_summary_html", "quick_wins_html", "risks_html",
            "recommendations_html", "roadmap_html", "vision_html",
            "one_pager_html", "sections_html", "praxisbeispiel_html",
            "compliance_html", "tools_html", "foerderprogramme_html",
            "gamechanger_html", "ueber_mich_html", "glossary_html"
        ]
        for key in html_keys:
            content = report.get(key)
            if isinstance(content, str) and content:
                cleaned = _strip_kpi_overview(content)
                cleaned = _strip_internal_guidelines(cleaned)
                report[key] = cleaned
        # Also clean within dynamic funding and one-pager if present
        dyn = report.get("dynamic_funding_html")
        if isinstance(dyn, str) and dyn:
            report["dynamic_funding_html"] = _strip_internal_guidelines(_strip_kpi_overview(dyn))
        # Clean sections list elements if they are stored under keys like 'exec_summary' etc.
        for base_key in ["executive_summary", "praxisbeispiel", "compliance", "foerderprogramme", "tools", "gamechanger"]:
            val = report.get(base_key)
            if isinstance(val, str) and val:
                report[base_key] = _strip_internal_guidelines(_strip_kpi_overview(val))

        # Replace stray German terms in English reports.  Occasionally the LLM may emit
        # German words such as "Beratung" into the English version (e.g. "Lean AI
        # Delivery for Beratung").  For consistency, translate "Beratung" to
        # "consulting" when the report language is English.  Perform a simple
        # replacement on all HTML fields after other sanitisation.
        if lang == "en":
            for key in html_keys:
                content = report.get(key)
                if isinstance(content, str) and content:
                    report[key] = content.replace("Beratung", "consulting")
            for base_key in ["executive_summary", "praxisbeispiel", "compliance", "foerderprogramme", "tools", "gamechanger"]:
                val = report.get(base_key)
                if isinstance(val, str) and val:
                    report[base_key] = val.replace("Beratung", "consulting")
