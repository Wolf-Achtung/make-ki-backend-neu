...
def ensure_html(text: str, lang: str = "de") -> str:
    ...
    for ln in lines:
        if re.match(r"^[-•*]\s+", ln):
            ...
            html.append("<li>"+re.sub(r"^[-•*]\s+","",ln).strip()+"</li>"); continue
...
def build_extended_risks(data: dict, lang: str = "de") -> str:
    ...
    if dq in {"niedrig","low"}:
        risks.append("Die Datenqualität ist niedrig; unstrukturierte/lückenhafte Daten erschweren KI-Projekte." if lang.startswith("de")
                     else "Your data quality is low; unstructured or incomplete data hinder AI projects.")
    if (data.get("ai_roadmap") or "").lower() in {"nein","no"}:
        risks.append("Es fehlt eine klar definierte KI-Roadmap; Risiko ineffizienter Insellösungen."
                     if lang.startswith("de") else "No clearly defined AI roadmap; risk of inefficient point solutions.")
    if (data.get("governance") or "").lower() in {"nein","no"}:
        risks.append("Keine Daten-/KI-Governance; erhöhtes Rechts-/Prozessrisiko."
                     if lang.startswith("de") else "No data/AI governance; increased legal/process risk.")
    ...
    if inv in {"eher_zurueckhaltend","sehr_zurueckhaltend","rather_reluctant","very_reluctant"}:
        risks.append("Zurückhaltende Innovationskultur kann KI-Erfolg gefährden."
                     if lang.startswith("de") else "Reluctant innovation culture can jeopardise AI success.")
    if not risks: return ""
...
def build_chart_payload(data: dict, score_percent: int, lang: str = "de") -> dict:
    ...
    auto_map={"sehr_niedrig":1,"eher_niedrig":2,"mittel":3,"eher_hoch":4,"sehr_hoch":5,"very_low":1,"rather_low":2,"medium":3,"rather_high":4,"very_high":5}
    ...
    know_map={"keine":1,"grundkenntnisse":2,"mittel":3,"fortgeschritten":4,"expertenwissen":5,"none":1,"basic":2,"medium":3,"advanced":4,"expert":5}
...
def build_funding_table(data: dict, lang: str = "de", max_items: int = 6):
    ...
    targets={"solo":["solo","freelancer","einzel"],"team":["kmu","team","small"],"kmu":["kmu","sme"]}.get(size,[])
...
def build_dynamic_funding(data: dict, lang: str = "de", max_items: int = 5) -> str:
    ...
    for p in selected:
        name=p.get("Name",""); desc=(p.get("Beschreibung","") or "").strip(); link=p.get("Link",""); grant=p.get("Fördersumme (€)","")
        line=(f"<b>{name}</b>: {desc} – Förderhöhe: {grant}" if lang.startswith("de")
              else f"<b>{name}</b>: {desc} – Funding amount: {grant}")
...
def render_prompt(template_text: str, context: dict) -> str:
    def replace_join(m):
        key=m.group(1); sep=m.group(2); val=context.get(key.strip(),"")
...
def build_masterprompt(chapter: str, context: dict, lang: str = "de") -> str:
    search_paths = [f"prompts/{lang}/{chapter}.md", f"prompts_unzip/{lang}/{chapter}.md", f"{lang}/{chapter}.md",
                    f"de_unzip/de/{chapter}.md", f"en_unzip/en/{chapter}.md"]
    prompt_text=None
    for p in search_paths:
        if os.path.exists(p):
            try: prompt_text=load_text(p); break
            except Exception: continue
    if prompt_text is None: prompt_text=f"[NO PROMPT FOUND for {chapter}/{lang}]"
    ...
    if str(lang).lower().startswith("de"):
        ...
        if context.get("is_self_employed"):
            style += ("\n\nWICHTIG: Der/die Nutzer:in arbeitet SOLO-selbstständig. "
                      "Vermeide Empfehlungen, die nur für größere Unternehmen sinnvoll sind. "
        ...
        if context.get("is_self_employed"):
            style += ("\n\nIMPORTANT: The user is SOLO self-employed. Avoid advice for large orgs. "
...
def _chat_complete(messages, model_name: Optional[str], temperature: Optional[float] = None) -> str:
    ...
    if temperature is None: temperature=float(os.getenv("GPT_TEMPERATURE","0.3"))
    if not str(args["model"]).startswith("gpt-5"): args["temperature"]=temperature
    resp=client.chat.completions.create(**args)
    return resp.choices[0].message.content.strip()

def build_context(data: dict, branche: str, lang: str = "de") -> dict:
    context_path=f"branchenkontext/{branche}.{lang}.yaml"
    if not os.path.exists(context_path): context_path=f"branchenkontext/default.{lang}.yaml"
...
def add_innovation_features(context, branche, data):
    ...
    try: context["gamechanger_blocks"]=build_gamechanger_blocks(data, GAMECHANGER_FEATURES)
    except Exception: context["gamechanger_blocks"]=[]
    return context
...
def add_websearch_links(context, branche, projektziel):
    ...
    try:
        ...
        context["websearch_links_foerder"]=[]; context["websearch_links_tools"]=[]
    return context
...
def gpt_generate_section(data, branche, chapter, lang="de"):
    ...
    section_text=_chat_complete(
        messages=[
            {"role":"system","content":("Du bist TÜV-zertifizierter KI-Manager, KI-Strategieberater, Datenschutz- und Fördermittel-Experte. "
...
def distill_quickwins_risks(source_html: str, lang: str = "de") -> Dict[str, str]:
    ...
    if not html: return {"quick_wins_html":"", "risks_html":""}
...
def calc_score_percent(data: dict) -> int:
    ...
    score=0; max_score=51
    score += as_int_val(data.get("digitalisierungsgrad",1),1)
    auto_map={"sehr_niedrig":0,"eher_niedrig":1,"mittel":3,"eher_hoch":4,"sehr_hoch":5,"very_low":0,"rather_low":1,"medium":3,"rather_high":4,"very_high":5}
    ...
    know_map={"keine":0,"grundkenntnisse":1,"mittel":3,"fortgeschritten":4,"expertenwissen":5,"none":0,"basic":1,"medium":3,"advanced":4,"expert":5}
    ...
    if data.get("strategische_ziele") or data.get("strategic_goals"): score += 1
    try: percent=int((score/max_score)*100)
...
def generate_full_report(data: dict, lang: str = "de") -> dict:
    ...
    for chap in chapters:
        try: out[chap]=gpt_generate_section_html(data, branche, chap, lang=lang)
        except Exception as e: out[chap]=f"<p>[Fehler in Kapitel {chap}: {e}]</p>"
    ...
    out["quick_wins_html"]=q_r.get("quick_wins_html",""); out["risks_html"]=q_r.get("risks_html","")
    ...
    out["roadmap_html"]=out.get("roadmap",""); out["exec_summary_html"]=out.get("executive_summary","")
    parts=[]
    if out.get("vision"): parts.append(f"<h2>{'Visionäre Empfehlung' if str(lang).startswith('de') else 'Visionary Recommendation'}</h2>\n{out['vision']}")
    ...
    if out.get("praxisbeispiel"): parts.append(f"<h2>{'Praxisbeispiel' if str(lang).startswith('de') else 'Case Study'}</h2>\n{out['praxisbeispiel']}")
    out["sections_html"]="\n\n".join(parts)
    if wants_funding:
    ...
    if not out.get("tools_table"):
        ...
        for m in re.finditer(r'<a[^>]*href="([^"]+)"[^>]*>([^<]+)</a>', html_tools, re.I):
            ...
            if name and link: rows.append({"name":name.strip(),"usecase":"","cost":"","link":link})
...
def _jinja_env():
    return Environment(loader=FileSystemLoader("templates"), autoescape=select_autoescape(["html","htm"]), enable_async=False)

def _pick_template(lang: str) -> str:
    if str(lang).lower().startswith("de") and os.path.exists("templates/pdf_template.html"): return "pdf_template.html"
    if os.path.exists("templates/pdf_template_en.html"): return "pdf_template_en.html"
    return None
...
async def analyze_briefing(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if template_name:
        env=_jinja_env(); tmpl=env.get_template(template_name)
        footer_de=("TÜV-zertifiziertes KI-Management © {year}: Wolf Hohl · E-Mail: kontakt@ki-sicherheit.jetzt · "
                   "Alle Inhalte ohne Gewähr; dieses Dokument ersetzt keine Rechtsberatung.")
        footer_en=("TÜV-certified AI Management © {year}: Wolf Hohl · Email: kontakt@ki-sicherheit.jetzt · "
