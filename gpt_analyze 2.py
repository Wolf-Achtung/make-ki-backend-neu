...
def generate_full_report(data: dict, lang: str = "de") -> dict:
    ...
    try:
        if lang == "de":
            ...
            know_bm = know_bench
            summary_sentence = (f" Ihr Digitalisierungsgrad liegt bei {dig}% (Branchenmedian {dig_bm}%), "
                                f"der Automatisierungsgrad bei {aut}% (Branchenschnitt {aut_bm}%), "
                                f"die Papierlosigkeit bei {paper}% (Benchmark {paper_bm}%) "
                                f"und das KI‑Know‑how bei {know}% (Benchmark {know_bm}%).")
            ...
            paper_bm = paper_bench
            know_bm = know_bench
            summary_sentence = (f" Your digitalisation level is {dig}% (sector median {dig_bm}%), "
                                f"automation is {aut}% (sector average {aut_bm}%), "
                                f"paperless processes reach {paper}% (benchmark {paper_bm}%), "
                                f"and AI know‑how is {know}% (benchmark {know_bm}%).")
