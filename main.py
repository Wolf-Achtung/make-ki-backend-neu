...
# main.py — Gold-Standard Backend (Template-Only) — Hotfix 2025-09-22
# Fixes:
#   • PDF sender: JSON-minimal payload option, body-snippet logging, CSS @page stripping (optional), script stripping, size cap
#   • Startup sanity: if OPENAI_API_KEY missing → LLM_MODE=off; if TAVILY_API_KEY missing → ALLOW_TAVILY=0
#   • Template-Only rendering enforced (Analyzer HTML ignored)
#   • Health/diag endpoints
#   • Idempotency for PDF jobs
#
# ENV (excerpt)
#   LOG_LEVEL=info|debug
#   TEMPLATE_DIR=templates | TEMPLATE_DE=pdf_template.html | TEMPLATE_EN=pdf_template_en.html
#   PDF_SERVICE_URL=... | PDF_TIMEOUT=120 | PDF_POST_MODE=html|json
#   PDF_JSON_MINIMAL=1|0          # when json, send only {"html": ...}
#   PDF_STRIP_SCRIPTS=1|0         # remove <script>...</script>
#   PDF_STRIP_PAGE_AT_RULES=1|0   # remove @page {...} rules
#   PDF_MAX_BYTES=600000
#   LLM_MODE=hybrid|off | OPENAI_API_KEY=...
#   ALLOW_TAVILY=0|1 | TAVILY_API_KEY=...
#   IDEMP_DIR=/tmp/ki_idempotency | IDEMP_TTL_SECONDS=1800
...
if (os.getenv("LLM_MODE", "off").lower() != "off") and not os.getenv("OPENAI_API_KEY"):
    logger.warning("[LLM] OPENAI_API_KEY missing -> forcing LLM_MODE=off")
...
async def send_feedback_mail_async(data: Dict[str, Any], user_email_hdr: Optional[str], ua: str, ip: str) -> None:
    ...
    def _send():
        with _s.SMTP(SMTP_HOST, SMTP_PORT, timeout=15) as s:
            try: s.starttls()
...
def load_template(lang: str) -> str:
    ...
    return f"<!doctype html><html lang='{lang}'><head><meta charset='utf-8'><title>{title}</title><style>body{{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;margin:24px;}}</style></head><body><h1>{title}</h1><p>Fallback-Ansicht: Kein Template gefunden. Bitte Templates unter /templates bereitstellen.</p></body></html>"
...
def render_html_from_report(report: Dict[str, Any], lang: str) -> str:
    ...
    for k, v in report.items():
        token = "{{ " + str(k) + " }}"; out = out.replace(token, str(v) if v is not None else "")
    ...
    if analyze_fn:
        try:
            ...
            if isinstance(result, dict): report = result
                ...
                logger.warning("Analyzer returned raw HTML — ignored (Template-Only enforced)."); report = {}
        except Exception as e:
...
<body><h1>{APP_NAME}</h1><p>Alles läuft. Endpunkte:</p>
<ul>
...
PDF_STRIP_PAGE_AT_RULES = os.getenv("PDF_STRIP_PAGE_AT_RULES","1") in ("1","true","True")
PDF_MAX_BYTES = int(os.getenv("PDF_MAX_BYTES","600000"))
...
def _sanitize_html_for_pdf(html: str) -> str:
    ...
    if PDF_STRIP_PAGE_AT_RULES:
        t = re.sub(r"@page\\s*\\{[^}]*\\}", "", t, flags=re.I|re.S)
        t = re.sub(r"@page\\s*:[^{]+\\{[^}]*\\}", "", t, flags=re.I|re.S)
...
async def send_html_to_pdf_service(html: str, user_email: str, subject: str = "KI-Readiness Report", lang: str = "de", request_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    modes_try = [preferred, alternate, preferred]

    last_exc = None; last_status = None; last_body_snip = None
            ...
            try:
                if mode == "json":
                    if PDF_JSON_MINIMAL:
                        ...
                        payload = {"html": html, "to": user_email or "", "adminEmail": os.getenv("ADMIN_EMAIL",""),
                                   "subject": subject, "lang": lang, "rid": rid}
                    resp = await client.post(f"{PDF_SERVICE_URL}/generate-pdf", json=payload)
                else:
                    headers = {"X-Request-ID": rid, "X-User-Email": user_email or "", "X-Subject": subject, "X-Lang": lang,
                ...
                if 200 <= resp.status_code < 300:
                    ...
                    last_status = resp.status_code; last_body_snip = snippet
                    logger.error("[PDF] rid=%s attempt=%s mode=%s status=%s body_snip=%r", rid, attempt, mode, resp.status_code, snippet)
                    if attempt == 2:
                        html = _sanitize_html_for_pdf(html[: int(len(html)*0.8) ])
            except (httpx.ReadTimeout, httpx.ConnectTimeout) as e:
                last_exc = e; wait = 1.8 ** attempt
                logger.warning("[PDF] rid=%s timeout attempt %s/3 → retry in %.2fs", rid, attempt, wait); await asyncio.sleep(wait)
            except Exception as e:
                last_exc = e; logger.warning("[PDF] rid=%s unexpected on attempt %s: %s", rid, attempt, repr(e)); await asyncio.sleep(1.0)
